...
public class StrongPassword {
	public static int strongPasswordChecker(String s) {
		...
		if (arr.length < 6) {
			res += total_missing + Math.max(0, 3 - (arr.length + total_missing));

		} else {
			int over_len = Math.max(arr.length - 6, 0), left_over = 0;
			res += over_len;
	...
	public static void main(String[] args) {
		System.out.println(strongPasswordChecker("normal5G"));
